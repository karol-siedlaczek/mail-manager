...
class MailManager:
    ...
    def create(self, table_name, domain_name=None, email_address=None):  # selector may be id or full name
        ...
        if table_name == DOMAINS_TABLE:
            ...
            query = f"INSERT INTO {table_name} (name, domain_id, password) VALUES ('{user_name}', '{user_domain_id}', '{user_password}')"
            ...
            try:
                ...
                forwarding_user_id = self.database.select(f"""
                    ...
                    FROM {table_name}
                    JOIN {USERS_TABLE} ON {USERS_TABLE}.id = {table_name}.user_id 
                    JOIN {DOMAINS_TABLE} ON {DOMAINS_TABLE}.id = {USERS_TABLE}.domain_id
                    WHERE {USERS_TABLE}.name = '{user_input.split('@')[0]}' AND {DOMAINS_TABLE}.name = '{user_input.split('@')[1]}'""")
    ...
    def update(self, table_name, index):
        ...
        if new_value == old_value:
            raise IndexError(f'new value ({new_value if not hide_value else "****"}) is equal with old value ({old_value if not hide_value else "****"}) in "{column}" column for row with "{index}" index in "{table_name}" table, updating aborted')
        self.database.update(f"UPDATE {table_name} SET {column} = '{new_value}' WHERE id = {index}")
        print(f'value of "{column}" column in "{table_name}" table for row with id "{index}" updated from "{old_value if not hide_value else "****"}" to "{new_value if not hide_value else "****"}"')
        log.info(f'value of "{column}" column in "{table_name}" table for row with id "{index}" updated from "{old_value if not hide_value else "****"}" to "{new_value if not hide_value else "****"}"')

    def get_list(self, table_name, inactive, active, filter_value):
        query = None
        headers = None
    ...
    def get_row(self, table_name, index):
        ...
        if table_name == DOMAINS_TABLE:
            query = f"""
                ...
                FROM {table_name} 
                WHERE {DOMAINS_TABLE}.id = {index}"""
        elif table_name == USERS_TABLE:
            query = f"""
                SELECT CONCAT({USERS_TABLE}.name, '@', {DOMAINS_TABLE}.name) 
                FROM {table_name} 
                JOIN {DOMAINS_TABLE} ON {DOMAINS_TABLE}.id = {USERS_TABLE}.domain_id 
                WHERE {USERS_TABLE}.id = {index}"""
        elif table_name == FORWARDINGS_TABLE:
            query = f"""
    ...
    def get_password(email_address):
        ...
        while user_password != user_password_retyped:
            ...
            user_password = getpass(prompt=f'Enter the password for "{email_address}" user: ')
            user_password_retyped = getpass(prompt=f'Retype password for "{email_address}" user: ')
        return hashlib.sha512(user_password.encode('utf-8')).hexdigest()  # get hashed password
...
def parse_args():
    ...
    parser.add_argument('-a', '--add',
                        help='Select table which some can be added',
                        type=str,
                        choices=DEFAULTS['TABLE_CHOICES'])
    parser.add_argument('-u', '--update',
                        help='Select table which can be updated',
                        type=str,
                        choices=DEFAULTS['TABLE_CHOICES'])
    parser.add_argument('-d', '--delete',
                        help='Select table which can be deleted',
                        type=str,
                        choices=DEFAULTS['TABLE_CHOICES'])
    ...
    elif not args.list and args.filter:  # when filtering, but listing column not selected
        raise parser.error('invalid usage of -f/--filter argument, the -l/--list argument is required to filter')
    return args
    ...
    try:
        ...
        try:
            for table in args.list:
                print(mailManager.get_list(table, args.inactive, args.active, args.filter))
